...
// It can be placed in the `api` directory of your project.
...
// A helper function to create the detailed system instruction for the AI model.
...
export default async function handler(req: Request) {
    ...
    const { config, history, message } = await req.json();

    if (!config || !message) {
        return new Response('Missing config or message in request body', { status: 400 });
    }
    ...
    // Convert the message from our frontend format to the Gemini API format
    const messageParts: Part[] = message.map((part: { type: string, text?: string, data?: string, mimeType?: string }) => {
        if (part.type === 'text') {
    ...
    try {
        ...
        // Create a new ReadableStream to pipe the response from Gemini
